...
export default function QuestionListItem({ question, active, expanded, onClick, onToggleExpand }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="min-w-[30px] h-[30px] px-1.5 rounded-full bg-accent text-white font-bold text-xs flex items-center justify-center shrink-0">
          {question.number}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-ink leading-snug">{question.text}</p>
          <div className="flex items-center flex-wrap gap-1.5 mt-2">
            ...
            {hasFeedback && (
              <button
                ...
                className="w-6 h-6 rounded-full bg-surface hover:bg-border flex items-center justify-center transition shrink-0 ml-auto"
              >
                <ChevronDown
                  size={14}
      ...
      {hasFeedback && expanded && (
        <div className="mt-2.5 bg-white border border-border rounded-xl p-3.5">
